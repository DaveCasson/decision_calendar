...
class DecisionCalendar:
    def __init__(self, config_path='ross.yaml', 
        ...
        self.streamflow_data = self._load_and_process_data(streamflow_csv, 'discharge_cms')
        self.swe_data = self._load_and_process_data(swe_csv, 'SWE', date_col='Date')
    ...
    def _add_track(self, sector, track_config):
        ...
        if sector_month in track_config.get('months', []):
            ...
            if ttype in ["infill", "arrow", "line", "marker"]:
                # Existing code for static tracks
                self._add_static_track(sector, track_config)
            elif ttype == "data_plot":
                print(f"Adding data plot for {sector_month}")
    ...
    def _add_data_plot(self, sector, track_config):
        ...
        # Use fill_between with normalized data. vmin=0, vmax=1 maps y=0 to r_start and y=1 to r_end.
        track.fill_between(xs, p10_vals, p90_vals, fc=color_env, alpha=0.2, edgecolor='none', zorder=2, vmin=0, vmax=1)
    ...
    def create_plot(self, center_image=None):
        ...
        for sector in circos.sectors:
            ...
            if center_image is not None:
                sector.raster(center_image, r=0, size=0.1)
    ...
    def _add_legend(self, fig):
        ...
        bbox = self.plot_settings['legend']['bbox']
